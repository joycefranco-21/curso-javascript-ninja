...
*/
    ...
    const everyLong = brasil.every(function (estados) {
        return estados.length > 7;
    });
...
*/
    ...
    const isCearaIncluded = brasil.some(function (estados) {
        return estados === 'Ceará';
    });
...
*/
       const arrayMap = newBrasil.map(function (item) {
         return {
           id: item.id + 1,
...
*/
    const filter = arrayMap.filter(function (item) {
        return (item.id % 2) === 0;
    });
